...
pub fn parse_neighbor_advertisement(payload: &[u8]) -> Result<NeighborAdvertisement, NetProtoError> {
    // ICMPv6 NA: type=136, minimum length=24 bytes
    if payload.len() < 24 {
        return Err(NetProtoError::Parse("ICMPv6 NA payload too short".into()));
    }
    if payload[0] != 136 {
        return Err(NetProtoError::Parse("not an ICMPv6 Neighbor Advertisement".into()));
    }
    ...
    Ok(NeighborAdvertisement { target_addr, link_layer_addr })
}
