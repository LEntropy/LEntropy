...
import { networkApi, endpointsApi, type HostEntry, type IpRule } from '../api/client'
import { StatusBadge } from '../components/StatusBadge'
import { Search, RefreshCw, Wifi, WifiOff, AlertCircle, ShieldBan, Trash2, ToggleLeft, ToggleRight, Plus, X } from 'lucide-react'

// ── IP 차단 규칙 패널 ──────────────────────────────────────────────────────────

function AddIpRuleModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [ipCidr, setIpCidr] = useState('')
  const [action, setAction] = useState<'block' | 'quarantine'>('block')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    if (!ipCidr.trim()) { setError('IP 주소를 입력하세요.'); return }
    setLoading(true)
    try {
      await networkApi.createIpRule({ ip_cidr: ipCidr.trim(), action, note: note || undefined })
      onCreated()
      onClose()
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'IP 규칙 생성 실패'
      setError(msg)
    } finally {
      setLoading(false)
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-900">IP 차단 규칙 추가</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">IP 주소 <span className="text-gray-400 font-normal">(예: 192.168.0.5 또는 10.0.0.0/24)</span></label>
            <input type="text" value={ipCidr} onChange={e => setIpCidr(e.target.value)} autoFocus placeholder="192.168.0.100"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">처리 방식</label>
            <div className="flex gap-2">
              {(['block', 'quarantine'] as const).map(a => (
                <label key={a} className={`flex-1 text-center cursor-pointer px-3 py-2 rounded-lg border text-sm transition-colors ${action === a ? 'border-red-500 bg-red-50 text-red-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                  <input type="radio" name="action" value={a} checked={action === a} onChange={() => setAction(a)} className="sr-only" />
                  {a === 'block' ? '🚫 차단' : '⚠️ 격리'}
                </label>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="차단 이유 등"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
          </div>
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 text-sm border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">취소</button>
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50">{loading ? '추가 중...' : '규칙 추가'}</button>
          </div>
        </form>
      </div>
    </div>
...
function IpRulesPanel() {
  const { data: rules, reload } = useApi<IpRule[]>(networkApi.listIpRules)
  const [showAdd, setShowAdd] = useState(false)
  const [loading, setLoading] = useState<string | null>(null)

  const handleDelete = async (rule: IpRule) => {
    if (!confirm(`"${rule.ip_cidr}" 차단 규칙을 삭제하시겠습니까?`)) return
    setLoading(rule.id + 'del')
    try { await networkApi.deleteIpRule(rule.id); await reload() } finally { setLoading(null) }
  }

  const handleToggle = async (rule: IpRule) => {
    setLoading(rule.id + 'toggle')
    try {
      if (rule.enabled) await networkApi.disableIpRule(rule.id)
      else await networkApi.enableIpRule(rule.id)
      await reload()
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm text-gray-800">IP 직접 차단 규칙</span>
          <span className="text-xs text-gray-400">({(rules ?? []).length}개)</span>
        </div>
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
          <Plus className="h-3.5 w-3.5" /> IP 차단 추가
        </button>
      </div>
      {(rules ?? []).length === 0 ? (
        <div className="px-4 py-6 text-center text-sm text-gray-400">등록된 IP 차단 규칙이 없습니다.</div>
      ) : (
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">IP / CIDR</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500">처리</th>
              ...
              <th className="text-right px-4 py-2 text-xs font-medium text-gray-500">작업</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {(rules ?? []).map(rule => (
              <tr key={rule.id} className={`hover:bg-gray-50 transition-colors ${!rule.enabled ? 'opacity-50' : ''}`}>
                <td className="px-4 py-2.5 font-mono text-xs font-medium text-gray-800">{rule.ip_cidr}</td>
                <td className="px-4 py-2.5">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${rule.action === 'block' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {rule.action === 'block' ? '차단' : '격리'}
                  </span>
                </td>
                <td className="px-4 py-2.5 text-xs text-gray-500">{rule.note ?? '—'}</td>
                <td className="px-4 py-2.5">
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${rule.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                    {rule.enabled ? '활성' : '비활성'}
                  </span>
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => handleToggle(rule)} disabled={loading === rule.id + 'toggle'} title={rule.enabled ? '비활성화' : '활성화'} className="p-1.5 rounded text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors">
                      {rule.enabled ? <ToggleRight className="h-4 w-4 text-green-500" /> : <ToggleLeft className="h-4 w-4" />}
                    </button>
                    <button onClick={() => handleDelete(rule)} disabled={loading === rule.id + 'del'} className="p-1.5 rounded text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
      ...
      {showAdd && <AddIpRuleModal onClose={() => setShowAdd(false)} onCreated={reload} />}
    </div>
  )
}
